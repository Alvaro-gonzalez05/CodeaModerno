      ...
      <div className="relative z-10 text-center px-4 py-8">
        ...
        <Slide key={`buttons-${key}`} direction="up" triggerOnce={false} delay={200}>
          <div className="flex flex-col md:flex-row gap-4 justify-center mt-10 items-center">
             <button onClick={openModal} className="group flex items-center gap-3 px-8 py-4 bg-[hsl(76,85%,67%)] text-black text-lg font-bold rounded-full hover:scale-105 transition-transform duration-300">
                Empezar un proyecto
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5 group-hover:translate-x-1 transition-transform">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" />
                </svg>
             </button>
             <button className="px-8 py-4 bg-transparent border border-white text-white text-lg font-bold rounded-full hover:bg-[hsl(76,85%,67%)] hover:border-[hsl(76,85%,67%)] hover:text-black hover:scale-105 transition-all duration-300">
                Contáctanos
             </button>
          </div>
        </Slide>
