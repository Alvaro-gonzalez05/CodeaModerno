...
export default async function LoginPage({
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col justify-center items-center p-4 relative overflow-hidden selection:bg-[hsl(76,85%,67%)] selection:text-black">
      ...
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[hsl(76,85%,67%)] rounded-full -translate-y-1/2 translate-x-1/3 blur-[120px] opacity-10 pointer-events-none"></div>
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-white rounded-full translate-y-1/3 -translate-x-1/3 blur-[120px] opacity-5 pointer-events-none"></div>
      ...
      <AuthAnimationWrapper>
        ...
        <div className="bg-neutral-900/80 backdrop-blur-xl border border-white/10 rounded-[2rem] p-8 shadow-2xl relative overflow-hidden group auth-stagger">
          ...
          <h2 className="text-3xl font-black uppercase tracking-tighter mb-2 relative z-10">INICIAR SESIÓN</h2>
          <p className="text-xs font-bold tracking-widest text-[#c2f254] mb-8 uppercase relative z-10">PORTAL DE ADMINISTRACIÓN CODEA</p>

          <form action="/api/auth/login" method="POST" className="flex-1 flex flex-col w-full justify-center gap-5 text-white relative z-10 auth-stagger">
            ...
            <button className="bg-[hsl(76,85%,67%)] text-black rounded-xl px-4 py-4 font-black uppercase tracking-widest mt-4 hover:shadow-[0_0_30px_rgba(194,242,84,0.4)] hover:scale-[1.02] transition-all text-xs">
              Ingresar al Sistema
            </button>
            ...
            <div className="mt-4 pt-6 border-t border-white/10 text-center text-[10px] font-bold uppercase tracking-widest text-gray-500">
              ¿Nuevo empleado? <Link href="/register" className="text-white hover:text-[hsl(76,85%,67%)] transition-colors ml-1">Solicitar acceso</Link>
            </div>
          </form>
                </div>
      </AuthAnimationWrapper>
